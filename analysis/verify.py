...
TARGET_RULES: dict[str, dict[str, Any]] = {
    "tomcat": {
        ...
        "service_templates": [
            {
                "path": VERIFICATION_DIR / "service" / "tomcat.yaml",
                "related_titles": {"Apache Tomcat PUT JSP Upload Risk"},
            },
        ],
        "risk_templates": [
            {
                "path": VERIFICATION_DIR / "risk" / "tomcat-put.yaml",
                "related_titles": {"Apache Tomcat PUT JSP Upload Risk"},
            },
        ],
    },
    "redis": {
        ...
        "service_templates": [
            {
                "path": VERIFICATION_DIR / "service" / "redis.yaml",
                "related_titles": {"Redis Unauthorized Access", "Redis Replication Abuse RCE Risk"},
            },
        ],
        "risk_templates": [
            {
                "path": VERIFICATION_DIR / "risk" / "redis-unauth.yaml",
                "related_titles": {"Redis Unauthorized Access"},
            },
            {
                "path": VERIFICATION_DIR / "risk" / "redis-rce.yaml",
                "related_titles": {"Redis Replication Abuse RCE Risk"},
            },
        ],
    },
    "elasticsearch": {
        ...
        "service_templates": [
            {
                "path": VERIFICATION_DIR / "service" / "elasticsearch.yaml",
                "related_titles": {
        ...
        "risk_templates": [
            {
                "path": VERIFICATION_DIR / "risk" / "elasticsearch-groovy.yaml",
                "related_titles": {"Elasticsearch Groovy Sandbox Escape Risk"},
            },
        ],
    },
    "samba": {
        ...
        "service_templates": [
            {
                "path": VERIFICATION_DIR / "service" / "samba.yaml",
                "related_titles": {"Samba Service Exposure", "SambaCry Remote Code Execution Risk"},
            },
        ],
        "risk_templates": [],
    },
    "vsftpd": {
        ...
        "service_templates": [
            {
                "path": VERIFICATION_DIR / "service" / "vsftpd.yaml",
                "related_titles": {"FTP Plaintext Service Exposure", "vsftpd Backdoor Risk"},
            },
        ],
        "risk_templates": [
            {
                "path": VERIFICATION_DIR / "risk" / "vsftpd-backdoor.yaml",
                "related_titles": {"vsftpd Backdoor Risk"},
            },
        ],
    },
}
...
def _promote_to_suspected_if_needed(
    ...
    *,
    ...
    analysis_titles: set[str],
    service_verified: bool,
) -> dict[str, Any]:
    if parsed_result["status"] != "not_verified":
        return parsed_result

    matched_titles = sorted(analysis_titles.intersection(related_titles))
    ...
    if verification_type == "service":
        ...
        promoted["reason"] = "no direct nuclei service match, but related analyzer titles exist"
        promoted["matched_analysis_titles"] = matched_titles
        promoted["evidence"] = (
            f"{parsed_result['evidence']}; promoted to suspected because "
            f"analysis contains related titles: {', '.join(matched_titles)}"
        )
        return promoted
    ...
    if verification_type == "risk" and service_verified:
        ...
        promoted["status"] = "suspected"
        promoted["confidence"] = "medium"
        promoted["reason"] = "no direct nuclei risk match, but service verification and related analyzer titles exist"
        promoted["matched_analysis_titles"] = matched_titles
        promoted["evidence"] = (
            f"{parsed_result['evidence']}; promoted to suspected because "
            f"service was at least suspected and analysis contains related titles: {', '.join(matched_titles)}"
        )
        return promoted
...
def _save_verification(
    ...
    payload = {
        ...
        "matched_analysis_titles": parsed_result.get("matched_analysis_titles", []),
...
def _execute_template(
    ...
    *,
    ...
    related_titles = set(template_def.get("related_titles", set()))
...
def verify_scan(scan_id: str, target_type: str | None = None) -> dict[str, Any]:
    ...
    analysis_titles = _extract_analysis_titles(analysis_result)
    ...
    service_results = _execute_template_list(
        ...
        target_endpoint=target_endpoint,
        matched_port=matched_port,
        template_defs=rule["service_templates"],
    ...
    risk_results = _execute_template_list(
        ...
        target_endpoint=target_endpoint,
        matched_port=matched_port,
        template_defs=rule["risk_templates"],
